...
def makeSignalModel(w, model, refhpass, refhfail, options):
    ...
    if model == "voigt":
        ...
        w.factory("Voigtian::signal1(mass, mean1[90,80,100], width[2.495], sigma1[2,0.0,10])")
        #w.factory("Voigtian::signal2(mass, mean2[90,80,100], width,        sigma2[4,1.5,10])")
        w.factory("Voigtian::signal2(mass, mean2[90,80,100], width,        sigma2[4,0.0,15])")
        ...
        w.factory("CBShape::resol(mass,dm[0,-4,4], sigma[3,1,10], alpha[3., 0.5, 10], n[1,0.1,25])")
        #w.factory("CBShape::resol(mass,dm[0,-4,4], sigma[3,1,10], alpha[3., 0.5, 30], n[1,0.1,35])")
        ...
        w.factory("DoubleCB::resol(mass,dm[0,-4,4], sigma[3,1,10], alpha[3., 0.5, 10], n[1,0.1,25], alpha2[3., 0.5, 8], n2[1,0.1,100])")
        #w.factory("DoubleCB::resol(mass,dm[0,-4,4], sigma[0,0,10], alpha[3., 0.5, 30], n[1,-0.1,25], alpha2[2., 0.0, 20], n2[1,-10.0,10])")
        w.factory("FCONV::signal(mass,zBW,resol)")
        return ("signal","signal")
    elif model == "BWDCB2":
        w.factory("BreitWigner::zBW(mass,MZ[91.1876], GammaZ[2.495])")
        ...
        #w.factory("DoubleCB::resol2(mass,sum::dm2(dm,dmfail[0,-3,3]), prod::sigma2(sigma,sigmafail[1,0.5,2]), alpha, n, alpha2, n2)")
        w.factory("FCONV::signal(mass,zBW,resol)")
        ...
        if options.fineBinning > 1: 
            ...
            for (hin,hout) in (refhfail,refrfail), (refhpass,refrpass):
                ...
                if "/TH1Keys_cc.so" not in ROOT.gSystem.GetLibraries(): 
                    ...
                    ROOT.gROOT.ProcessLine(".L %s/src/TH1Keys.cc+" % os.environ['CMSSW_BASE']);
        ...
        if model == "MCT":
           ...
           w.factory("Gaussian::resol(mass,dm[0,-4,4], sigma[0.1,0.01,5])")
            ...
            w.factory("CBShape::resol(mass,dm[0,-4,4], sigma[0.1,0.01,10], alpha[3., 0.5, 20], n[1,0.1,35])")
        elif model == "MCTDCB":
            #w.factory("DoubleCB::resol(mass,dm[0,-4,4], sigma[0.1,0.01,10], alpha[3., 0.5, 8], n[1,0.1,25], alpha2[3., 0.5, 8], n2[1,0.1,100])")
            w.factory("DoubleCB::resol(mass,dm[0,-4,4], sigma[0.1,0.01,10], alpha[3., 0.5, 20], n[1,0.1,35], alpha2[3., 0.5, 8], n2[1,0.1,100])")
        ...
        w.factory("SUM::signal(gFrac[0.5,0,1.0]*signal1, signal2)")     #TJ 14.05.2018
        return ("signal","signal")
    elif model == "JCB":
...
#        w.factory("RooChebychev2::backgroundFail(mass, {a0[0.25,0,0.5], a1[0,-1,0.1],a2[0.,-0.25,0.25]})")
    ...
    if model == "cmsShape":
	w.factory("RooCMSShape::backgroundPass(mass, alphaPass[70.,60.,90.], betaPass[0.02, 0.01,0.1], gammaPass[0.001, 0.,0.1], peakPass[90.0])")
        w.factory("RooCMSShape::backgroundFail(mass, alphaFail[70.,60.,90.], betaFail[0.02, 0.01,0.1], gammaFail[0.001, 0.,0.1], peakPass)")
        return ("backgroundPass", "backgroundFail")

    if model == "cmsShapebeta0p2":
	w.factory("RooCMSShape::backgroundPass(mass, alphaPass[70.,60.,90.], betaPass[0.001, 0.,0.1], gammaPass[0.001, 0.,0.1], peakPass[90.0])")
#        w.factory("RooCMSShape::backgroundPass(mass, alphaPass[70.,60.,92.], betaPass[0.001, 0.,0.15], gammaPass[0.001, 0.,0.15], peakPass[90.0])")
#        w.factory("RooCMSShape::backgroundFail(mass, alphaFail[70.,60.,92.], betaFail[0.03, 0.02,0.15], gammaFail[0.001, 0.,0.15], peakPass)")
	w.factory("RooCMSShape::backgroundFail(mass, alphaFail[70.,60.,90.], betaFail[0.03, 0.02,0.1], gammaFail[0.001, 0.,0.1], peakPass)")
        return ("backgroundPass", "backgroundFail")


    #elif model in ("bern1","bern2","bern3","bern4","bern5","bern6","bern7"):
    elif model in ("bern1","bern2","bern3","bern4","bern5","bern6","bern7", "bern8","bern9","bern10","bern11","bern12","bern13","bern14","bern15","bern16","bern17","bern18","bern19","bern20","bern21","bern22","bern23","bern24","bern25"):   #test
        order = int(model.replace("bern",""))
        #cpass = ",".join(("prod::c2_pass%d(c_pass%d[0.2,-1,1],c_pass%d)"%(i,i,i)) for i in xrange(order))
        cpass = ",".join(("prod::c2_pass%d(c_pass%d[0.2,-0.1,2],c_pass%d)"%(i,i,i)) for i in xrange(order))
        #cfail = ",".join(("prod::c2_fail%d(c_fail%d[0.2,-1,1],c_fail%d)"%(i,i,i)) for i in xrange(order))
        cfail = ",".join(("prod::c2_fail%d(c_fail%d[0.2,-0.1,2],c_fail%d)"%(i,i,i)) for i in xrange(order))
...
#        ROOT.gROOT.ProcessLine(".L %s/src/functions.cc+" % os.environ['CMSSW_BASE']);
    ...
    if not os.path.exists(options.printDirBins):
        ...
        os.system("cp index.php  %s/" % options.printDirBins)
